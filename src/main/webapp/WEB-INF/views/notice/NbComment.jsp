			...
			success : function(data) {
				...
				$.each(
					...
					function(key, value) {
						...
						if(m_Id == value.m_Id){
							a += '<a class="btn btn-sm" onclick="commentUpdate('+ value.nb_C_Num +',\''+ value.nb_C_Content +'\');"> 수정 </a>';
							a += '<a class="btn btn-sm" onclick="commentDelete('+ value.nb_C_Num +');"> 삭제 </a> </div>';
						} else if(m_Id == m_Id2 || grade == 'admin'){
							a += '<a class="btn btn-sm" onclick="commentDelete('+ value.nb_C_Num +');"> 삭제 </a> </div>';
						} else{
							a += '</div>';
						}
						a += '<div class="commentContent'+value.nb_C_Num+'"> '+ value.m_Id + '<p> 내용 : '+ value.nb_C_Content+ '</p>';
						a += '</div></div>';
					}
				);
				$(".commentList").html(a);
